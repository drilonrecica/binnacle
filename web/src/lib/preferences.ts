...
export function resolveTheme(theme: Theme, dark = matchMedia('(prefers-color-scheme: dark)').matches): 'dark' | 'light' {
  return theme === 'system' ? (dark ? 'dark' : 'light') : theme;
}

export function preferences(storage: Storage = localStorage): { theme: Theme; density: Density } {
  ...
  return {
    ...
    density: density === 'compact' || density === 'comfortable' ? density : 'comfortable',
  };
}

export function applyPreferences(value: { theme: Theme; density: Density }, storage: Storage = localStorage) {

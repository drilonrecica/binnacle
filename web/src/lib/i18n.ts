...
export function formatNumber(
  ...
  return value == null ? t('unavailable') : formatter(locale).format(value);
}
export function formatBytes(value: number | null | undefined, locale?: string) {
  ...
  return `${formatter(locale).format(value)} ${units[index]}`;
}
export function formatRate(value: number | null | undefined, locale?: string) {
  return value == null ? t('unavailable') : `${formatBytes(value, locale)}/s`;
}
...
export function formatDuration(
  ...
  return seconds == null
    ...
    : new Intl.NumberFormat(locale, {
        style: 'unit',
        unit: seconds >= 3600 ? 'hour' : seconds >= 60 ? 'minute' : 'second',

...
dev: ## Start the demo development environment (requires T006 and T015).

	@printf '%s\n' 'make dev is unavailable: complete T006 (application lifecycle) and T015 (deterministic demo collector) first.' >&2
	@printf '%s\n' 'Those tasks will use a fresh temporary TALOS_DATA_DIR for each development run.' >&2
	@exit 1

dev-demo: ## Run the deterministic demo environment (requires T006 and T015).

	@printf '%s\n' 'make dev-demo is unavailable: complete T006 (application lifecycle) and T015 (deterministic demo collector) first.' >&2
...
dev-host: ## Run against local host/Docker interfaces (requires collectors).
	...
	@printf '%s\n' 'make dev-host is unavailable: complete T006, T031, and T043 before using real host or Docker interfaces.' >&2
